...
/*                                                                                    */
/* Analysis script for our RBC transfusion study. This code:                          */
...
/* CSV output directory - auto-create if missing */
OPTIONS DLCREATEDIR;
%LET csv_outpath=<PROJECT_ROOT>\analysis\script_data;

/* Clean up WORK library from any previous incomplete runs */
...
%let dataset_name = %sysfunc(ifc(%length(&sysparm)>0, &sysparm, transvital3_sample));
...
DATA work.transvital_raw;
    ...
    TransfusionID=CATX('_', PatientID, PUT(TransDt, E8601DT19.), Poolnum,
        UnitNumber, ComponentType);
RUN;
...
DATA work.transfusions;
    ...
    IF MISSING(DonorHb_storage) AND NOT MISSING(DonorHb) THEN DonorHb_storage=
        DonorHb;
    IF MISSING(DonorSex_storage) AND NOT MISSING(DonorSex) THEN DonorSex_storage
        =DonorSex;
    IF MISSING(DonorParity_storage) AND NOT MISSING(DonorParity) THEN
        DonorParity_storage=DonorParity;
    IF MISSING(Storage_storage) AND NOT MISSING(Storage) THEN Storage_storage=
        Storage;
    ...
    KEEP TransfusionID PatientID DonorHb DonorSex DonorParity Storage TransDt
        Datetime Age Sex admissiontime dischargetime wdy_donation
        DiagnosisCodeName_1;
RUN;
...
DATA work.transfusions;
    ...
    LENGTH donorhb_category $7 storagecat $7 donor_sex_label $1
        donor_parity_label $10;
    ...
    KEEP TransfusionID PatientID DonorHb DonorSex DonorParity Storage TransDt
        Datetime Age Sex admissiontime dischargetime wdy_donation
        DiagnosisCodeName_1 donorhb_category storagecat donor_sex_label
        donor_parity_label;
RUN;
...
PROC SQL;
    CREATE TABLE unique_patients_count AS SELECT COUNT(DISTINCT PatientID) AS
        No_of_Unique_Patients FROM work.transfusions;
QUIT;
...
PROC EXPORT DATA=unique_patients_count
    OUTFILE="&csv_outpath\unique_patients_count.csv" DBMS=CSV REPLACE;
RUN;

/* Patient sex distribution */
PROC SQL;
    CREATE TABLE patient_sex_distribution AS SELECT Sex AS Patient_Sex,
        COUNT(DISTINCT PatientID) AS No_of_Patients FROM work.transvital_raw
        GROUP BY Sex;
QUIT;
...
PROC EXPORT DATA=patient_sex_distribution
    OUTFILE="&csv_outpath\patient_sex_distribution.csv" DBMS=CSV REPLACE;
RUN;

/* Patient age statistics */
PROC SQL;
    CREATE TABLE unique_patients_age AS SELECT DISTINCT PatientID, Age FROM
        work.transvital_raw;
QUIT;

PROC MEANS DATA=unique_patients_age;
    VAR Age;
    OUTPUT OUT=age_stats MEAN=Mean_Age MEDIAN=Median_Age MIN=Min_Age MAX=Max_Age
        Q1=Q1_Age Q3=Q3_Age;
RUN;
...
/* Export to CSV */
PROC EXPORT DATA=age_stats OUTFILE="&csv_outpath\patient_age_stats.csv" DBMS=CSV
    REPLACE;
RUN;
...
/* Export to CSV */
PROC EXPORT DATA=age_group_counts OUTFILE="&csv_outpath\patient_age_groups.csv"
    DBMS=CSV REPLACE;
RUN;
...
PROC SQL;
    CREATE TABLE patients_units AS SELECT PatientID, COUNT(TransfusionID) AS
        No_of_Units FROM work.transfusions GROUP BY PatientID;
QUIT;
...
PROC SQL NOPRINT;
    SELECT COUNT(DISTINCT PatientID) INTO :total_pt FROM
        patients_unit_categories;
QUIT;
...
/* Export to CSV */
PROC EXPORT DATA=units_freq OUTFILE="&csv_outpath\rbc_units_per_patient.csv"
    DBMS=CSV REPLACE;
RUN;

/* Show total patients */
PROC SQL;
    SELECT COUNT(DISTINCT PatientID) AS Unique_patients FROM
        patients_unit_categories;
QUIT;
...
/* Total units count */
PROC SQL;
    CREATE TABLE total_transfused_units AS SELECT COUNT(*) AS
        No_of_Transfused_Units FROM work.transfusions;
QUIT;
...
PROC EXPORT DATA=total_transfused_units
    OUTFILE="&csv_outpath\total_transfused_units.csv" DBMS=CSV REPLACE;
RUN;

/* Donor Hb distribution */
PROC SQL;
    CREATE TABLE donorhb_distribution AS SELECT donorhb_category, COUNT(*) AS
        No_of_Transfused_Units FROM work.transfusions GROUP BY donorhb_category;
QUIT;
...
PROC EXPORT DATA=donorhb_distribution
    OUTFILE="&csv_outpath\donorhb_distribution.csv" DBMS=CSV REPLACE;
RUN;

/* Storage time distribution */
PROC SQL;
    CREATE TABLE storage_distribution AS SELECT storagecat, COUNT(*) AS
        No_of_Transfused_Units FROM work.transfusions GROUP BY storagecat;
QUIT;
...
PROC EXPORT DATA=storage_distribution
    OUTFILE="&csv_outpath\storage_distribution.csv" DBMS=CSV REPLACE;
RUN;

/* Donor sex distribution */
PROC SQL;
    CREATE TABLE sex_distribution AS SELECT donor_sex_label, COUNT(*) AS
        No_of_Transfused_Units FROM work.transfusions GROUP BY donor_sex_label;
QUIT;
...
PROC EXPORT DATA=sex_distribution
    OUTFILE="&csv_outpath\donor_sex_distribution.csv" DBMS=CSV REPLACE;
RUN;

/* Donor parity distribution */
PROC SQL;
    CREATE TABLE donorparity_distribution AS SELECT donor_parity_label, COUNT(*)
        AS No_of_Transfused_Units FROM work.transfusions GROUP BY
        donor_parity_label;
QUIT;
...
PROC EXPORT DATA=donorparity_distribution
    OUTFILE="&csv_outpath\donor_parity_distribution.csv" DBMS=CSV REPLACE;
RUN;

/* Donation weekday distribution */
PROC SQL;
    CREATE TABLE wdy_distribution AS SELECT wdy_donation, COUNT(*) AS
        No_of_Transfused_Units FROM work.transfusions GROUP BY wdy_donation;
QUIT;
...
PROC EXPORT DATA=wdy_distribution
    OUTFILE="&csv_outpath\donation_weekday_distribution.csv" DBMS=CSV REPLACE;
RUN;
...
libname plots "&plot_path";

/* Direct all graph output to the plots folder */
ODS LISTING GPATH="&plot_path";
ODS GRAPHICS ON / IMAGENAME="Descriptive";
...
/***************************************************************************/
/*==========================================================================*/
/*  CLEANUP - Remove temporary files and SAS datasets                       */
/*==========================================================================*/

/* Clean up WORK library - remove all temporary SAS datasets */
PROC DATASETS LIB=WORK NOLIST KILL;
QUIT;
